...
public class StringUtil {
    ...
    /**
     * 用换行和逗号分隔字符串，且去重,去空字符串
     * @param value
     * @return
     */
    public static String[] stringToDistinctArray(String value){
        return stringToDistinctArray(value, "\n|\r\n|,");
    }

    /**
     * 用正则分隔字符串，且去重,去空字符串
     * @param value 待分隔字符串
     * @param regex 分隔字符串的正则字符串
     * @return
     */
    public static String[] stringToDistinctArray(String value, String regex){
        if(StringUtils.isBlank(value) || StringUtils.isBlank(regex)){
            return null;
        }
        String[] tempArr = value.split(regex);
        Set<String> setResult = new HashSet<String>(tempArr.length);
        for(String temp : tempArr){
            if(StringUtils.isNotBlank(temp)){
                setResult.add(temp);
            }
        }
        String[] result = new String[setResult.size()];
        return setResult.toArray(result);
    }
	...
	public static String encrypt(String message,String PASSWORD_CRYPT_KEY,String PASSWORD_CRYPT_IV) throws Exception {
		...
		Base64 base64Encoder = new Base64();
  		return new String(base64Encoder.encode(pasByte),"utf-8");
	}
