	...
	*/
	...
	public static void setCookie(HttpServletResponse response,Serializable serializable)
	{
		try {
			...
			RemoteCacheUtil.setValue(ticket, value);
			Jedis jedis = RemoteCacheUtil.getJedis();
			// 一天过期
			jedis.expire("outside"+ticket, 60 * 60 * 24);
			RemoteCacheUtil.returnResource(jedis);
			CookieUtil.setCookie(response, MD5Util.getMD5String(DateUtil.getCurrentDate("yyyy-MM-dd")), ticket, -1);
		} catch (Exception e) {
		}
	}
	...
	public static void deleteCookie(HttpServletRequest request,HttpServletResponse response)
	{
		...
		try {
			...
			// 从缓存删除ticket
			RemoteCacheUtil.remove(ticket);
		} catch (NoSuchAlgorithmException e) {
			e.printStackTrace();
		}
		try {
			deleteCookie(response, new Cookie(MD5Util.getMD5String(DateUtil.getCurrentDate("yyyy-MM-dd")),null));
		} catch (NoSuchAlgorithmException e) {
