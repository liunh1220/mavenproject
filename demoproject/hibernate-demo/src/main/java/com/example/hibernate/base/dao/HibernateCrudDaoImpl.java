package com.example.hibernate.base.dao;
...
/**
 * Hibernate CRUD DAO implement
 * @author liu.nh
 *
 * @param <T>
 * @param <PK>
 */
public class HibernateCrudDaoImpl<T extends Serializable, PK extends Serializable> 
		implements IHibernateCrudDao<T, PK> {
	...
	public T loadWithLock(PK id, LockMode lock) {
		T t = (T) hibernateTemplate.load(entityClass, id, lock);
		if (t != null) {
			this.flush(); // 立即刷新，否则锁不会生效。
		}
		return t;
	...
	public Criteria createCriteria() {
		return this.createDetachedCriteria().getExecutableCriteria(hibernateTemplate.getSessionFactory().getCurrentSession());
	}
	...
	public List<T> findEqualByEntity(T entity, String[] propertyNames) {
		...
		exam.excludeZeroes();
		String[] defPropertys = hibernateTemplate.getSessionFactory().getClassMetadata(entityClass).getPropertyNames();
	...
	public Transaction beginTransaction() {
		return hibernateTemplate.getSessionFactory().getCurrentSession().beginTransaction();
	}
